...
def main():
    ...
    try:
        ...
        if os.path.exists(vocals_path):
            ...
            # Step 3: Quantize pitch map to MIDI note blocks
            pitch_map_path = os.path.join(output_dir, "pitch_map.json")
            quantized_path = os.path.join(output_dir, "pitch_quantized.json")
            try:
                quantize_cmd = [
                    sys.executable,
                    os.path.join(os.path.dirname(__file__), "pitch_quantizer.py"),
                    pitch_map_path,
                    quantized_path
                ]
                print(f"[karaoke_worker] Running pitch quantizer: {' '.join(quantize_cmd)}", flush=True)
                quantize_result = subprocess.run(quantize_cmd, capture_output=True, text=True, timeout=60)
                if quantize_result.returncode != 0:
                    print(f"[karaoke_worker] Pitch quantizer stderr: {quantize_result.stderr}", flush=True)
                else:
                    print(f"[karaoke_worker] Pitch quantizer stdout: {quantize_result.stdout}", flush=True)
            except Exception as e:
                print(f"[karaoke_worker] Pitch quantizer failed: {e}", flush=True)
        else:
