...
module.exports = {
        ...
        try {
            ...
            switch (buttonType) {
                ...
                case 'music_seek_back':
                    await this.handleSeekBack(interaction, player, requesterId);
                    break;

                case 'music_seek_forward':
                    await this.handleSeekForward(interaction, player, requesterId);
                    break;
    ...
    async handleSkip(interaction, player, requesterId) {
        ...
        if (skipped) {
            ...
            if (player.ui && player.currentTrack) {
                await player.ui.updateNowPlayingEmbed();
            }
        } else {
            await interaction.reply({
    ...
    async handlePrevious(interaction, player, requesterId) {
        ...
        if (result) {
            const embed = new EmbedBuilder()
                .setTitle(await LanguageManager.getTranslation(interaction.guild?.id, 'buttonhandler.moved_to_previous_title'))
                .setDescription(`**[${player.currentTrack.title}](${player.currentTrack.url})** ${await LanguageManager.getTranslation(interaction.guild?.id, 'buttonhandler.now_playing')}!`)
                .setColor(config.bot.embedColor)
                .setTimestamp()
                .addFields({
                    name: await LanguageManager.getTranslation(interaction.guild?.id, 'buttonhandler.action_by'),
                    value: `${interaction.member}`,
                    inline: true
                });
    ...
    async handleSeekBack(interaction, player, requesterId) {
        // Authorization check
        if (!this.isAuthorized(interaction, requesterId)) {
            return await interaction.reply({
                content: await LanguageManager.getTranslation(interaction.guild?.id, 'buttonhandler.not_authorized'),
        ...
        if (typeof player.seek !== 'function') {
            return await interaction.reply({
                content: await LanguageManager.getTranslation(interaction.guild?.id, 'buttonhandler.seek_not_available'),
                flags: [1 << 6]
            });
        }

        const currentTime = player.getCurrentTime ? player.getCurrentTime() : 0;
        const newPosition = Math.max(0, currentTime - 10000); // 10 seconds back

        const result = player.seek(newPosition);

        if (result) {
            await interaction.reply({
                content: await LanguageManager.getTranslation(interaction.guild?.id, 'buttonhandler.seek_backward_success'),
                flags: [1 << 6]
            });

            // Update main embed to show new position
            if (player.ui) {
                await player.ui.updateNowPlayingEmbed();
            }
        } else {
            await interaction.reply({
                content: await LanguageManager.getTranslation(interaction.guild?.id, 'buttonhandler.seek_failed'),
                flags: [1 << 6]
            });
        }
    },

    async handleSeekForward(interaction, player, requesterId) {
        // Authorization check
        if (!this.isAuthorized(interaction, requesterId)) {
            return await interaction.reply({
                content: await LanguageManager.getTranslation(interaction.guild?.id, 'buttonhandler.not_authorized'),
        ...
        const currentTime = player.getCurrentTime ? player.getCurrentTime() : 0;
        const trackDuration = player.currentTrack.duration * 1000;
        const newPosition = Math.min(trackDuration, currentTime + 10000); // 10 seconds forward

        const result = player.seek(newPosition);

        if (result) {
            await interaction.reply({
                content: await LanguageManager.getTranslation(interaction.guild?.id, 'buttonhandler.seek_forward_success'),
                flags: [1 << 6]
            });
    ...
    async handleSearchInteraction(interaction, client) {
        ...
        try {
            ...
            const result = await player.handleMusicData(
                {
                    isPlaylist: false,
